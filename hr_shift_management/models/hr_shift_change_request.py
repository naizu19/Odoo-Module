...
class HrShiftChangeRequest(models.Model):
        ...
        for request in self:
            request.can_approve = (
                ...
                and (
                    ...
                    or user.has_group('hr_shift_management.group_shift_officer')
                )
            )
    ...
    def action_submit(self):
        for request in self:
            ...
            request._send_shift_mail('hr_shift_management.mail_template_shift_submit')
    ...
    def action_approve(self):
        for request in self:
            ...
            request._send_shift_mail(
                'hr_shift_management.mail_template_shift_approved',
    ...
    def action_reject(self):
        for request in self:
            ...
            partners = request._get_approval_notify_partners()
            request._send_shift_mail(
                'hr_shift_management.mail_template_shift_rejected',
                partner_ids=partners.ids,
            )
    ...
    def action_cancel(self):
        for request in self.filtered(lambda r: r.state in ('draft', 'submitted')):
            ...
            if was_submitted:
                ...
                request._send_shift_mail(
                    'hr_shift_management.mail_template_shift_cancelled',
                    partner_ids=partners.ids,
                )
    ...
    def _schedule_approval_activity(self):
        ...
        if self.approver_id:
            self.activity_schedule(
                'hr_shift_management.mail_activity_shift_approval',
    ...
    def _done_approval_activities(self):
        activity_type = self.env.ref(
            'hr_shift_management.mail_activity_shift_approval', raise_if_not_found=False,
        )
