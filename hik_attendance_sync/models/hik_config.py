...
class HikConfig(models.Model):
    ...
    webhook_token = fields.Char(string='Webhook Token', help="Security token sent in headers to verify authenticity",
                                default=lambda self: self._default_webhook_token())
    ...
    def _get_signature(self, method, url_path, headers):
        ...
        custom_headers = {k.lower(): v for k, v in headers.items() if
                          (k.lower().startswith('x-ca-') or k.lower() == 'userid')}
    ...
    def _call_hik_api(self, url_path, payload):
        ...
        try:
            ...
            return {'error': True, 'msg': _("Connection to %s timed out. Check your firewall/network.") % self.base_url,
                    'execution_time': (time.time() - start_time) * 1000}
        except requests.exceptions.ReadTimeout:
            return {'error': True, 'msg': _("Server at %s is responding too slowly.") % self.base_url,
                    'execution_time': (time.time() - start_time) * 1000}
        except requests.exceptions.ConnectionError:
            return {'error': True,
                    'msg': _("Connection error to %s. The server might be down or URL is wrong.") % self.base_url,
                    'execution_time': (time.time() - start_time) * 1000}
        except requests.exceptions.HTTPError as e:
            return {'error': True, 'msg': _("HTTP Error: %s") % str(e),
                    'execution_time': (time.time() - start_time) * 1000}
        except Exception as e:
            return {'error': True, 'msg': _("Request Exception: %s") % str(e),
                    'execution_time': (time.time() - start_time) * 1000}
        ...
        for config in self:
            ...
            for etype in event_type_selection:
                ...
                while True:
                    ...
                    if result['error']:
                        config._log_sync_error(result['msg'], result.get('data'),
    ...
    def action_unsubscribe_events(self):
        ...
        for config in self:
            ...
            result = config._call_hik_api("/artemis/api/eventService/v1/eventUnSubscriptionByEventTypes",
